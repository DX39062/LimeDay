...
2.7.2 的冻结修改、兼容边界与验收清单见 [docs/V2_7_2_CHANGE_SPEC.md](docs/V2_7_2_CHANGE_SPEC.md)。

## 下载

- [LimeDay 2.7.2 ARM64 APK](releases/LimeDay-2.7.2-arm64-v8a.apk)
...
- 一级待办分组、内置“日常”、16 个手绘分组图标、可排序步骤，以及标题/备注/步骤本地搜索
- 当天、逾期、计划中智能视图和按分组折叠显示
- 不闪退的默认折叠搜索、仅当天显示的紧凑完成进度，以及语义不重复的自绘待办菜单图标
- 快速新增栏可在加号左侧仅用手绘图标选择分组，并保留本次连续录入选择
...
- 以日记页、待办行和青柠切片为主体的涂鸦应用图标、待办页顶部装饰、自绘开关，以及全套 Compose Canvas 自绘导航与操作图标
- 跟随系统/浅色/深色外观模式和每日待办、复盘提醒
- JSON 数据导入导出、版本信息和隐私说明
- CC Switch 风格的多模型服务卡片管理、16 个服务预设、各厂商独立手绘 Logo、同厂商多配置、固定编辑入口、默认切换、复制、排序和连接检查
- 模型服务页提供页头与列表底部双新增入口，先选择预设再编辑，不覆盖已有服务
...
- MainActivity 退出 Android 预测返回预览，侧边拖动不再提前移动或缩放应用页面
