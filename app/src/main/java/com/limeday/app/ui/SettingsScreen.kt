        ...
        LazyColumn(
            ...
            item {
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    Text("WebDAV 同步", style = MaterialTheme.typography.headlineLarge)
            ...
            item {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    OutlinedButton(onClick = { onTest(draft) }, modifier = Modifier.weight(1f), enabled = !state.isTestingWebDav) {
                        if (state.isTestingWebDav) CircularProgressIndicator(modifier = Modifier.size(18.dp), strokeWidth = 2.dp)
            ...
            item {
                Button(
                    ...
                    if (state.isSyncing) CircularProgressIndicator(modifier = Modifier.size(18.dp), strokeWidth = 2.dp)
                ...
                item {
                    Surface(shape = RoundedCornerShape(12.dp), color = MaterialTheme.colorScheme.surfaceContainer) {
                        Column(Modifier.fillMaxWidth().padding(14.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            ...
                            state.metadata?.lastSyncAt?.let { Text("最近记录：${formatTime(it)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
                        }
                    }
                }
            }
