...
Azure OpenAI、AWS Bedrock 和 Vertex AI 需要专门的认证字段、签名流程或资源路径，当前 2.6 不用不完整的“兼容模式”冒充支持。
...
2.6 要求新建服务在首次保存前同样提供显眼的“手动获取模型”按钮。应用不因 API Key 输入自动联网；只有用户点击后才向当前端点请求。端点或必要凭据缺失时按钮禁用，失败不阻止手填与保存。
