# LimeDay 2.7–2.7.2 待办领域模型与 WebDAV v2
...
- `groupId`：TodoGroup UUID；旧数据迁入固定 ID 的内置“日常”。
- `dueDate`：可选 ISO 日期。
...
TodoGroup 是同步实体，包含名称、`iconKey`、`colorKey`、`sortOrder`、`isInbox` 和同步元数据。每个安装必须存在一个固定 ID 的内置“日常”；内部字段 `isInbox` 仅为兼容名称。用户不能删除该内置分组。

2.7.1 新安装直接创建“日常”。从 2.7.0 升级时，仅当固定 ID、`isInbox = true`、未删除且名称严格等于“收件箱”时改名；固定 ID、Todo 引用、Room schema 和 WebDAV 格式均不改变。重复执行不得再次写入或递增 revision，用户已经改过的其他分组不受影响。

删除普通分组采用软删除。其 Todo 不批量改写，读取与显示时将无效 groupId 解析为“日常”；用户之后编辑 Todo 时可写回固定默认分组 ID。这样删除分组不会制造大量同步冲突。

待办页按有效分组顺序显示区段，折叠状态只保存在本机 UI 状态。逾期、计划中和搜索结果可以按分组继续分区。

### 2.7.2 分组图标

`iconKey` 继续是同步字段，不改变表结构或 WebDAV v2。2.7.2 提供稳定的内置键：`daily`、`work`、`study`、`home`、`health`、`shopping`、`finance`、`travel`、`idea`、`project`、`habit`、`reading`、`sport`、`chores`、`social`、`other`。未知键显示 `other` fallback，但序列化时不得静默改写未知原值。

固定“日常”显示 `daily`。新普通分组按有效分组当前已用图标，从固定列表中选择第一个未使用键；全部已使用时循环复用。用户在创建或编辑时可显式更换图标与颜色。

2.7.1 及更早版本自动写入的空键或通用 `folder`、`leaf` 占位键可在 2.7.2 首次规范化时按 `sortOrder`、`id` 稳定顺序分配。规范化只处理已知占位值，保留明确键、分组 ID、名称、颜色、排序、删除状态和 Todo 引用；写入正常同步元数据并确保再次启动不重复修改。

### 2.7.2 快速新增分组

快速新增在加号左侧显示当前分组图标。UI 会话初始值为固定“日常”ID，成功添加后继续保留当前选择，进程重启不持久化该临时选择。提交前必须确认目标分组仍有效；缺失或已删除时回退到“日常”。Repository 新增方法接收显式 `groupId`，但数据库字段和同步格式不变。

## 步骤

TodoStep 是独立同步实体，包含 `todoId`、标题、完成状态、排序与同步元数据。删除步骤采用软删除；主 Todo 软删除时步骤保留以支持恢复，主 Todo 永久删除时步骤正文物理清除。

搜索在 Room 中覆盖 Todo 标题/备注和有效步骤标题。首版不实现全文索引，以转义后的 `LIKE` 查询满足本地规模；结果按逾期、未完成、计划日和稳定排序排列。2.7.2 修复只涉及 Compose 焦点状态：输入框必须获得过焦点后，空查询失焦才折叠；不修改 Room 查询与结果排序。
...
3. v1 存在则解析为内存 v2：生成/复用固定 ID 的“日常”，所有 Todo 指向该分组，新集合为空。
